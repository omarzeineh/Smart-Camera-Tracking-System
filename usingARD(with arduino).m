...
while true
    ...
    if ~trackingActive
        ...
        oldX = errorX;
        oldY = errorY;
        errorX = size(frame, 2)/2 - Xmean;
        errorY = size(frame, 1)/2 - Ymean;
        dt = toc;
        integralX = integralX+errorX*dt;
        integralY = integralY+errorY*dt;
        dX = (errorX-oldX);
        dY = (errorY-oldY);
        angleXInc = Kp*errorX + Ki*integralX + Kd*dX;
        angleYInc = Kp*errorY + Ki*integralY + Kd*dY;
        angleX = angleX-angleXInc;
        angleY = angleY-angleYInc;
        if angleX > 90
            angleX=90;
        end
        if angleX < -90
        ...
        posX = min(max(posX, 0), 1);
        posY = min(max(posY, 0), 1);
        
        writePosition(sX, posX);
        writePosition(sY, posY);
